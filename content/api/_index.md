...
Videre kreves en API-nøkkel ("subscription key") for den eller de tjenestene som skal benyttes. Disse kan man be om gjennom utviklerportalen på [data.altinn.no](https://data.altinn.no).
...
Gamle versjoner vil bli støttet i minst 12 måneder etter at en ny versjon tilgjengeliggjøres, og alle registrerte brukere vil bli underrettet i god tid før utfasing av gamle versjoner finner sted for å sikre at alle får migrert til ny versjon.
...
Det er ulike base-URL-er som skal benyttes avhengig av miljø.
...
Det er to miljøer som er tilgjengelige utenfra: prod og test. Begge miljøene har et metadata-API, som ikke krever autentisering eller autorisasjon for bruk.
